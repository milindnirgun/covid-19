...
with st.beta_container():
    latestDate = latest_us_df_date.strftime("%m/%d/%Y")
    col1, col2 = st.beta_columns(2)
    with col1:
        st.markdown("### Confirmed Cases")
        st.write("**" + '{:,}'.format(cases[0]) + "**  for " + latestDate)
        st.write("**" + '{:,}'.format((cases[0] - prev_cases[1])) + "**  increased from previous day")

    with col2:
        st.markdown("### Reported Deaths")
        st.markdown("**" + '{:,}'.format(deaths[0]) + "**  for " + latestDate)
        st.markdown("**" + '{:,}'.format((deaths[0] - prev_deaths[1])) + "**  increased from previous day")
#st.markdown('  ___  ')
